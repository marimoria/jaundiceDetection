...
Imported lazily (only when debug=True) so it never slows down batch runs.
"""
...
def save_debug_figure(
    ...
    skin_mask       : np.ndarray   Binary mask (255 = skin).
    ...
    plt.close(fig)
    LOG.info("debug figure saved: %s", out_path)
...
def _draw_feature_text(ax, features: dict) -> None:
    ...
    ax.text(0.05, 0.95, text, transform=ax.transAxes, fontsize=7.5,
            verticalalignment="top", fontfamily="monospace", color=_TEXT_COL)
...
def _draw_rgb_histogram(ax, skin_pixels_rgb: np.ndarray) -> None:
    if len(skin_pixels_rgb) > 0:
        for ch, col, lbl in [(0, "#ff6b6b", "R"), (1, "#6bff9e", "G"), (2, "#6bb5ff", "B")]:
            ax.hist(skin_pixels_rgb[:, ch], bins=50, color=col, alpha=0.6, label=lbl, density=True)
